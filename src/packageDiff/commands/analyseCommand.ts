...
interface AnalyseDeps {
  cache: CacheService
  diffCalculator: DiffCalculator
}

export function createAnalyseCommand({ cache, diffCalculator }: AnalyseDeps): Command {
  const command = new Command('analyse')
  ...
  command.action(async () => {
    ...
    try {
      ...
      const showProgress = shouldShowProgress(format, options)
      if (showProgress) {
...
function shouldShowProgress(format: string, options: Record<string, unknown>): boolean {
  const noProgress = Boolean(options.noProgress)
  return format === 'text' && process.stdout.isTTY === true && !noProgress
}
